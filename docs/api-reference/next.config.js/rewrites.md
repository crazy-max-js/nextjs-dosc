...
Rewrites act as a URL proxy and mask the destination path, making it appear the user hasn't changed their location on the site. In contrast,[redirects](/docs/guide/api-reference/next.config.js/redirects)will reroute to a new page and show the URL changes.
...
- [headers](/docs/guide/api-reference/next.config.js/headers)are checked/applied- [redirects](/docs/guide/api-reference/next.config.js/redirects)are checked/applied- `beforeFiles`rewrites are checked/applied- static files from the[public directory](/docs/guide/basic-features/static-file-serving),`_next/static`files, and non-dynamic pages are checked/served- `afterFiles`rewrites are checked/applied, if one of these rewrites is matched we check dynamic routes/static files after each match- `fallback`rewrites are checked/applied, these are applied before rendering the 404 page and after dynamic routes/all static assets have been checked. If you use[fallback: true/'blocking'](/docs/guide/api-reference/data-fetching/get-static-paths#fallback-true)in`getStaticPaths`, the fallback`rewrites`defined in your`next.config.js`willnotbe run.
...
Note: for static pages from the[Automatic Static Optimization](/docs/guide/advanced-features/automatic-static-optimization)or[prerendering](/docs/guide/basic-features/data-fetching/get-static-props)params from rewrites will be parsed on the client after hydration and provided in the query.
...
:::details 示例
- [Incremental adoption of Next.js](https://github.com/vercel/next.js/tree/canary/examples/custom-routes-proxying)- [Using Multiple Zones](https://github.com/vercel/next.js/tree/canary/examples/with-zones)
:::
...
See additional information on incremental adoption[in the docs here](/docs/guide/migrating/incremental-adoption).

### Rewrites with basePath support

When leveraging[`basePath`support](/docs/guide/api-reference/next.config.js/basepath)with rewrites each`source`and`destination`is automatically prefixed with the`basePath`unless you add`basePath: false`to the rewrite:
