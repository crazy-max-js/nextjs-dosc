...
- Make sure to keep only the pages you need in your app, meaning that an app can't have pages from another app, if app`A`has`/blog`then app`B`shouldn't have it too.- Make sure to configure a[basePath](/docs/guide/api-reference/next.config.js/basepath)to avoid conflicts with pages and static files.

## How to merge zones

You can merge zones using[`rewrites`](/docs/guide/api-reference/next.config.js/rewrites)in one of the apps or any HTTP proxy.
