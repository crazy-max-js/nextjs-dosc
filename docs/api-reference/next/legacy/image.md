...
When using`layout="intrinsic"`or`layout="fixed"`the`height`property represents therenderedheight in pixels, so it will affect how large the image appears.
...
> Images can not be optimized at build time using[`next export`](/docs/advanced-features/static-html-export), only on-demand. To use`next/legacy/image`with`next export`, you will need to use a different loader than the default.[Read more in the discussion.](https://github.com/vercel/next.js/discussions/19065)
